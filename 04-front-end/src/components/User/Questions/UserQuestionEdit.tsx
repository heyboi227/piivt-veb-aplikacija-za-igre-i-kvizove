...
export interface IUserQuestionEditUrlParams extends Record<string, string | undefined> {
    qid: string
}
...
type TAddAnswerFull = { type: "editQuestionForm/addAnswerFull", value: { answerId: number, isCorrect: boolean } };
type TRemoveAnswer = { type: "editQuestionForm/removeAnswer", value: number };
type TSetAnswerIsCorrect = { type: "editQuestionForm/setAnswerIsCorrect", value: { answerId: number, isCorrect: boolean } };

type EditQuestionFormAction = TSetGameId
    | TSetTitle
    | TAddAnswer
    | TAddAnswerFull
    | TRemoveAnswer
...
function EditQuestionFormReducer(oldState: IEditQuestionFormState, action: EditQuestionFormAction): IEditQuestionFormState {
    switch (action.type) {
...
export default function UserQuestionEdit() {
    const params = useParams<IUserQuestionEditUrlParams>();
    ...
    const [formState, dispatchFormStateAction] = useReducer(EditQuestionFormReducer, {
        gameId: 1,
        title: "",
        answers: [],
    ...
    const loadQuestion = () => {
        api("get", "/api/question/" + questionId, "activeUser")
            .then(res => {
                if (res.status !== "ok") {
    ...
    const loadAnswers = () => {
        api("get", "/api/answer/game/" + formState.gameId, "activeUser")
            .then(res => {
                if (res.status !== "ok") {
    ...
    const doEditQuestion = () => {
        api("put", "/api/question/" + questionId, "activeUser", formState)
            .then(res => {
                if (res.status !== "ok") {
                    throw new Error("Could not edit this question! Reason: " + res?.data?.map((error: any) => error?.instancePath + " " + error?.message).join(", "));
                }
    ...
    useEffect(() => {
        dispatchFormStateAction({ type: "editQuestionForm/setGameId", value: question?.gameId ?? 1 });
        dispatchFormStateAction({ type: "editQuestionForm/setTitle", value: question?.title ?? '' });

        for (let answer of question?.answers ?? []) {
            dispatchFormStateAction({
                type: "editQuestionForm/addAnswerFull", value: {
                    ...
                    <div className="card-text">
                        ...
                        <div className="row">
                            <div className="col col-12 col-lg-7 mb-3 mb-lg-0">
                                ...
                                <div className="form-group mb-3">
                                    ...
                                    <div className="input-group">
                                        <input type="number" className="form-control form-control-sm"
                                            ...
                                            onChange={e => dispatchFormStateAction({ type: "editQuestionForm/setGameId", value: +e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="form-group mb-3">
                                    ...
                                    <div className="input-group">
                                        <textarea className="form-control form-control-sm" rows={5}
                                            ...
                                            onChange={e => dispatchFormStateAction({ type: "editQuestionForm/setTitle", value: e.target.value })}
                                        />
                                    </div>
                                </div>
                                        ...
                                        return (
                                            <div className="row" key={"answer-" + answer.answerId}>
                                                <div className="col col-3">
                                                    {
                                                        answerData
                                                            ? <FontAwesomeIcon onClick={() => dispatchFormStateAction({ type: "editQuestionForm/removeAnswer", value: answer.answerId })} icon={faCheckSquare} />
                                                            : <FontAwesomeIcon onClick={() => dispatchFormStateAction({ type: "editQuestionForm/addAnswer", value: answer.answerId })} icon={faSquare} />
                                                    } {answer.answerValue}
                                                </div>
                                                        ...
                                                        <>
                                                            <div className="col col-2">
                                                                <div className="input-group input-group-sm">
                                                                    <select onChange={e => dispatchFormStateAction({ type: "editQuestionForm/setAnswerIsCorrect", value: { answerId: answer.answerId, isCorrect: e.target.value === "correct" ? true : false } })} defaultValue={answerData.isCorrect ? "correct" : "incorrect"}>
                                ...
                                <div className="form-froup mb-3">
                                    <button className="btn btn-primary" onClick={() => doEditQuestion()}>
                                        Edit question
                                    </button>
                                </div>
                            </div>
