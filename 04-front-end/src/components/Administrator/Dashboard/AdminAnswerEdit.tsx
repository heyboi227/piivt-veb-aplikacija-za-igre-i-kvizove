...
export interface IAdminAnswerEditUrlParams extends Record<string, string | undefined> {
    aid: string
}
...
type EditAnswerFormAction = TSetGameId
    | TSetAnswerValue;

function EditAnswerFormReducer(oldState: IEditAnswerFormState, action: EditAnswerFormAction): IEditAnswerFormState {
    switch (action.type) {
...
export default function AdminAnswerEdit() {
    ...
    const [formState, dispatchFormStateAction] = useReducer(EditAnswerFormReducer, {
        gameId: 1,
        answerValue: "",
    });
    ...
    const doEditAnswer = () => {
        api("put", "/api/answer/" + answerId, "administrator", formState)
            .then(res => {
                if (res.status !== "ok") {
                    throw new Error("Could not edit this answer! Reason: " + res?.data?.map((error: any) => error?.instancePath + " " + error?.message).join(", "));
                }
    ...
    useEffect(() => {
        dispatchFormStateAction({ type: "editAnswerForm/setGameId", value: answer?.gameId ?? 1 });
        dispatchFormStateAction({ type: "editAnswerForm/setAnswerValue", value: answer?.answerValue ?? '' });
                                ...
                                <div className="form-group mb-3">
                                    ...
                                    <div className="input-group">
                                        <input type="number" className="form-control form-control-sm"
                                            ...
                                            onChange={e => dispatchFormStateAction({ type: "editAnswerForm/setGameId", value: +e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="form-group mb-3">
                                    ...
                                    <div className="input-group">
                                        <textarea className="form-control form-control-sm" rows={5}
                                            ...
                                            onChange={e => dispatchFormStateAction({ type: "editAnswerForm/setAnswerValue", value: e.target.value })}
                                        />
                                    </div>
                                </div>

                                <div className="form-froup mb-3">
                                    <button className="btn btn-primary" onClick={() => doEditAnswer()}>
                                        Edit answer
                                    </button>
                                </div>
                            </div>
